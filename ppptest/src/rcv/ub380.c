...
#define ID_GPSEPHEM 7           /* message id: ub380 decoded gps ephemeris */
...
#define ID_BDSEPHEMERIS 1047    /* message id: ub370/ub380 decoded bds ephemeris */

/*add by liyong*/
#define ID_SATVIS 48 /*Satellite Visibility*/
...
/*decode satellite visibility*/
static int decode_satvis(raw_t *raw) 
{
	...
	int prn;
	//FILE * satFp;
	double elev, az, appdop;

	trace(3, "decode_satvis: len=%d\n", raw->len);

	satVis = U4(p);
	p = p + 8; //ingore satvis compalm

	nobs = U4(p);

	if (raw->len < OEM4HLEN + 12 + nobs * 40) 
	{
		trace(2, "oem4 satvis length error: len=%d nobs=%d\n", raw->len, nobs);
		return -1;
	}

	if ((satVis == 0) || (nobs == 0))
		return 0; //no visible sat

	//satFp = fopen("/var/www/cgi-bin/satvis.txt",O_WRONLY|O_CREAT);
	//fprintf(satFp,"satvis:%d ",nobs);
	...
	for(i = 0, p += 4; i < nobs; i++, p += 40) 
	{
		...
		// SYS_CMP,beidou
		/* obs position */
		p = p + 6; //ignore glofreq&health

		elev = R8(p);
		az = R8(p + 8);
		p = p + 24; //ignore True dop
		appdop = R8(p);
		
		//fprintf(satFp,"prn:%d elev:%lf az:%lf appdop:%lf\n",prn, elev, az, appdop);
...
static int decode_rangecmpb(raw_t *raw)
{
    ...
    for (i=0,p+=4;i<nobs;i++,p+=24) {
	    ...
	    if (sys==SYS_CMP) prn-=160;//ub370/ub380中输出的BDS数据中（包括原始数据和导航电文），卫星prn为161~197，而novtael中bds的prn为1~35
...
/* decode bdsephemerisb  ub370/ub380------------------------------------------------------*/
static int decode_bdsephemerisb(raw_t *raw)
{
	...
	//ub370/ub380中输出的BDS数据中（包括原始数据和导航电文），卫星prn为161~197，而novtael中bds的prn为1~35
	prn-=160;
	...
	//此处ub370/ub380中输出的week是按照gps起始时间算的，故不能使用bdt2time而只能使用gpst2time
    eph.toe=bdt2gpst(gpst2time(eph.week,eph.toes)); /* bdt -> gpst */  
    eph.toc=bdt2gpst(gpst2time(eph.week,toc));      /* bdt -> gpst */
    eph.ttr=raw->time;
...
///UB370中输出的BDS数据中（包括原始数据和导航电文），卫星prn为161~197，而novtael oem6中bds的prn为1~35。
///gps，glo，bds的星历对应的信息id分别为41,723,1047，而Novatel oem6中为41,723,1069，故而增加了对id为1047的信息
///的解码。UB370中bds的导航电文中的时间系统使用的是gpst而不是bdt,故求解星历数据时注意转换

///UB380和UB370数据类型基本相同，不同的是ub380中gps导航电文对应的命令是log gpsephemb onnew（对应的信息id是7）
///而不是log rawephemb onnew（对应的信息id是41）,故解码UB380数据时需要增加对id为7的信息的解码

/* decode ub370/ub380 message -------------------------------------------------------*/
static int decode_ub380(raw_t *raw)
{
    double tow;
    ...
    switch (type) {
		...
		case ID_GPSEPHEM      : return decode_gpsephemerisb  (raw);//ub380
        ...
        case ID_BDSEPHEMERIS  : return decode_bdsephemerisb  (raw); //ub370/ub380
    }
    return 0;
}
/* decode oem3 message -------------------------------------------------------*/
...
/* input ub370/ub380 raw data from stream ----------------------------------------
* fetch next novatel ub370/ub380 raw data and input a mesasge from stream
...
*
*ub370/ub380的一个完整数据包和oem6一致，故数据同步标记，以及crc校验和oem6一致
*-----------------------------------------------------------------------------*/
extern int input_ub380(raw_t *raw, unsigned char data)
{
    ...
    if (raw->nbyte==10&&(raw->len=U2(raw->buff+8)+OEM4HLEN)>MAXRAWLEN-4) {
        trace(2,"ub380 length error: len=%d\n",raw->len);
        raw->nbyte=0;
        return -1;
    }
    ...
    /* decode ub380 message */
    return decode_ub380(raw);
}
