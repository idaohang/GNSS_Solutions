...
extern int myRTpos( /*const prcopt_t *popt, const solopt_t *sopt,*/ char* ntrip_str) {
	...
	strsvrinit(&mysvr_obs, n);  //用于接收rtcm观测数据的server
	strsvrinit(&mysvr_ssr, n);  //用于接收ssr改正数的server
	//启动服务线程
	...
	myprcopt.tidecorr = 2;  // 改正固体潮汐，极移改正,海洋潮汐等
	...
	myprcopt.tropopt = TROPOPT_EST; //估计ZTD
	//myprcopt.tropopt = TROPOPT_SAAS ; //估计ZTD

	myprcopt.mode = PMODE_PPP_KINEMA;
	//myprcopt.mode = PMODE_SINGLE;
	myprcopt.modear = 4;  //4:ppp-ar
	myprcopt.sateph = EPHOPT_SSRAPC;  //这里的apc和com的区别是卫星天线相位中心是否进行改正
	myprcopt.nf = 1;  //设置单频
	...
	//禁用卫星
	//myprcopt.exsats[4] = 1;  //排除掉17号卫星
	//myprcopt.exsats[9] = 1;  //排除掉13号卫星
...
extern int mypost(const prcopt_t *popt, const solopt_t *sopt) {
	...
	int i, nobs, n, solstatic, pri[] = { 0, 1, 2, 3, 4, 5, 1, 6 };
	FILE* logfile = NULL;
	char blqfilepath[MAXSTRPATH] = "blq.dat";  //海洋潮blq文件
	char timestr[50] = { 0 };
	...
	readpcv("data/igs08.atx", &pcvsr);  //读取测站的天线pcv文件；由于atx文件中同时包含了卫星和接收机的相位中心数据，全部读到pcvsr中

	//设置卫星和接收机的天线相位中心
	setpcv(obss.n > 0 ? obss.data[0].time : timeget(), popt, &navs, &pcvsr, &pcvsr, stas); //因此这里统一用pcvsr

	//读取码间偏差dcb文件,不能用于载波相位数据的延迟处理
	readdcb("data/P1P21501.DCB", &navs);

	//readotl(popt,blqfilepath,stas);  //读取海洋潮汐blq文件

	//解算完成后调用antpos进行接收机偏心改正
	...
	//设置rb-2267750.574	5009156.111	3221291.833
	...
	//此处函数参考procpos函数这里的nobs实际上是卫星个数
	while ((nobs = inputobs(obs, rtk.sol.stat, popt)) >= 0) {
		num++;
