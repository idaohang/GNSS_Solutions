...
/* get socket error ----------------------------------------------------------*/
#ifdef WIN32
static int errsock(void) { /*return WSAGetLastError();*/ return 0; }
#else
...
#ifdef WIN32
    u_long mode=1; 
    int err , myerr =-1,retVal =-1;
    //设置为非阻塞模式
    myerr =  ioctlsocket(sock,FIONBIO,&mode);
    
	if(myerr == SOCKET_ERROR)
	{
		return -1;
	}
	if ( retVal=connect(sock,addr,len)== INVALID_SOCKET ) 
	{
        //err=errsock();
		err = WSAGetLastError();
...
static int gentcp(tcp_t *tcp, int type, char *msg)
{
	...
	{ /* client socket */
		...
		//memcpy(&tcp->addr.sin_addr, &destAddr, sizeof(destAddr));
	}
    tcp->state=1;
    tcp->tact=tickget();
...
static int consock(tcpcli_t *tcpcli, char *msg)
{
    ...
    /* non-block connect */
    if (  (stat=connect_nb(tcpcli->svr.sock,(struct sockaddr *)&tcpcli->svr.addr,sizeof(tcpcli->svr.addr)))==-1 ) 
	{
        err=errsock();
...
#ifdef WIN32
    test = WSAStartup(MAKEWORD(2,0),&data);
#endif
