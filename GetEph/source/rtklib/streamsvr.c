...
static int is_navmsg(int msg) {
	return msg == 1019 || msg == 1020 || msg == 1044 || msg == 1045 || msg == 1046 || msg == 1047;
}
...
extern strconv_t *strconvnew(int itype, int otype, const char *msgs, int staid,
	...
	memset(conv, 0, sizeof(strconv_t));
...
static void raw2rtcm(rtcm_t *out, const raw_t *raw, int ret) {
	...
	if (ret == 1) {
		...
		case SYS_GPS: out->nav.eph[sat - 1] = raw->nav.eph[sat - 1]; break;
		case SYS_GAL: out->nav.eph[sat - 1] = raw->nav.eph[sat - 1]; break;
		case SYS_QZS: out->nav.eph[sat - 1] = raw->nav.eph[sat - 1]; break;
...
static void rtcm2rtcm(rtcm_t *out, const rtcm_t *rtcm, int ret, int stasel) {
	...
	} else if (ret == 2)   //星历
	{
		...
		case SYS_GPS: out->nav.eph[sat - 1] = rtcm->nav.eph[sat - 1]; break;
		case SYS_GAL:
		case SYS_QZS:
...
static int nextsat(nav_t *nav, int sat, int msg) {
	...
	case 1045: sys = SYS_GAL; p1 = MINPRNGAL; p2 = MAXPRNGAL; break;
	case 1047: sys = SYS_CMP; p1 = MINPRNCMP; p2 = MAXPRNCMP; break;
	//case 1045: sys = SYS_CMP; p1 = MINPRNCMP; p2 = MAXPRNCMP; break;
...
extern int strsvrstart(strsvr_t *svr, int *opts, int *strs, char **paths,
	...
	/*-----when the buffsize<0, malloc return NULL-----*/
	...
	for (i = 0; i<svr->nstr; i++) {
		...
		if (strs[i] != STR_FILE) rw |= STR_MODE_W;  //rw = STR_MODE_RW
