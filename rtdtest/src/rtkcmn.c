                   ...
                   double *xp, double *Pp)
{
	...
	fprintf(g_mylog,"num of obs: %d num of x: %d \n",m,n);
	fprintf(g_mylog,"initial value for the filter->x:\n");
	for( i = 0 ; i< n; i++ )
	{
		fprintf(g_mylog,"%8.3f ",x[i]);
	}
	fprintf(g_mylog,"\n");
	fprintf(g_mylog,"initial dev  for the filter->P:\n");
	for( i = 0 ; i< n; i++ )
	{
		for( j = 0 ; j< n; j++ )
		{
			fprintf(g_mylog,"%8.3f ",P[i*n+j]);
		}
		fprintf(g_mylog,"\n");
	}
    
	fprintf(g_mylog,"measurement value  for the filter->v:\n");
	for( i = 0 ; i< m; i++ )
	{
		fprintf(g_mylog,"%8.3f ",v[i]);
	}
	fprintf(g_mylog,"\n");
	fprintf(g_mylog,"measurement dev  for the filter->R:\n");
	for( i = 0 ; i< m; i++ )
	{
		for( j = 0 ; j< m; j++ )
		{
			fprintf(g_mylog,"%8.3f ",R[i*m+j]);
		}
		fprintf(g_mylog,"\n");
	}

	fprintf(g_mylog,"design matrix for the filter->H:\n");
	for( i = 0 ; i< m; i++ )
	{
		for( j = 0 ; j< n; j++ )
		{
			fprintf(g_mylog,"%8.3f ",H[i*n+j]);
		}
		fprintf(g_mylog,"\n");
	}
	...
	fprintf(g_mylog,"filter value->xp:\n");
	for( i = 0 ; i< n; i++ )
	{
		fprintf(g_mylog,"%8.3f ",xp[i]);
	}
	fprintf(g_mylog,"\n");
	fprintf(g_mylog,"filtered vaule dev->Pp:\n");
	for( i = 0 ; i< n; i++ )
	{
		for( j = 0 ; j< n; j++ )
		{
			fprintf(g_mylog,"%8.3f ",Pp[i*n+j]);
		}
		fprintf(g_mylog,"\n");
	}
